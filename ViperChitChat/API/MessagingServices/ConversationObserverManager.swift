...
class ConversationObserverManager {
    ...
    // MARK:- Private
    fileprivate func fetchChatPartner(fromID: String, toID: String, completion: @escaping (_ user: ChitChatUser) -> Void) {
        var id: String!
    ...
    fileprivate func synthesisThread(thread: [String: Any], completion: @escaping (_ thread: Thread) -> Void) {
        if let metaData = thread["metaData"] as? [String: Any] {
            guard let lastMessagePayload = metaData["lastMessage"] as? [String: Any],
                  let fromID = lastMessagePayload["fromID"] as? String,
                let messageID = metaData["lastMessageID"] as? String,
                let messageSeen = metaData["lastMessageSeen"] as? Bool,
                let timestamp = lastMessagePayload["timestamp"] as? Double,
                let toID = lastMessagePayload["toID"] as? String else { return }
            self.fetchChatPartner(fromID: fromID, toID: toID) { (user) in
                if let text = lastMessagePayload["text"] as? String {
