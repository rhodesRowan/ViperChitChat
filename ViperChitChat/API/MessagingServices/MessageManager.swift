...
class MessageManager {
    ...
    public func uploadVideo(videoData: Data,
                            completion: @escaping (_ success: String?,
        _ thumbnailImageURL: String?, _ thumbnailAspect: Double?) -> Void) {
        let uid = NSUUID().uuidString + ".mov"
        let videoPath = storageRef.child("messageVideos").child(uid)
        videoPath.putData(videoData, metadata: nil) { (_, error) in
            if let err = error {
                print(err.localizedDescription)
