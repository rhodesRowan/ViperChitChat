...
class ChatLogObserverManager {
    ...
    public func getPreviousMessagesForThread(chatPartnerID: String,
                                             firstTimestamp: Double,
                                             completion: @escaping (_ messages: [BaseMessage]) -> Void) {
        guard let user = AuthManager.sharedInstance.getCurrentUser() else { return }
        var tempMessages = [BaseMessage]()
        let group = DispatchGroup()
        let threadRef = databaseRef.child("user-messages")
            .child(user.uid).child(chatPartnerID).child("messages")
            .queryOrdered(byChild: "timestamp").queryEnding(atValue: firstTimestamp).queryLimited(toLast: 30)
    ...
    public func observeInitialMessagesForThread(chatPartnerID: String,
                                                completion: @escaping (_ message: [BaseMessage]) -> Void) -> DatabaseReference? {
        ...
        threadRef.queryOrdered(byChild: "timestamp").queryLimited(toLast: 30)
    ...
    public func observeNewMessagesForThread(chatPartnerID: String,
                                            completion: @escaping (_ message: BaseMessage?) -> Void) -> DatabaseReference? {
        guard let user = AuthManager.sharedInstance.getCurrentUser() else { return nil}
        let threadRef = databaseRef.child("user-messages").child(user.uid).child(chatPartnerID).child("messages")
        threadRef.queryOrdered(byChild: "timestamp")
            .queryLimited(toLast: 1).observe(.childAdded) { [weak self] (snapshot) in
            guard let self = self else { return }
    ...
    private func fetchMessage(messageID: String, completion: @escaping(_ message: BaseMessage) -> Void) {
        ...
        messageRef.observeSingleEvent(of: .value) { (snapshot) in
            if let dict = snapshot.value as? [String: Any],
                ...
                let fromID = dict["fromID"] as? String, let date = dict["timestamp"] as? Double {
                if let text = dict["text"] as? String {
                    let textMessage = TextMessage(toID: toID, fromID: fromID,
                                                  text: text, date: date,
                                                  messageID: messageID, type: .textMessageCell)
                    completion(textMessage)
                } else if let imageURL = dict["imageURL"] as? String {
                    let photoMessage = PhotoMessage(toID: toID, fromID: fromID,
                                                    date: date, messageID: messageID,
                                                    imageURL: imageURL,
                                                    type: .imageMessageCell)
                    completion(photoMessage)
                } else if let gifID = dict["gifID"] as? String {
                    let gifMessage = GifMessage(toID: toID, fromID: fromID,
                                                date: date, messageID: messageID,
                                                gifID: gifID, type: .gifMessageCell)
                    completion(gifMessage)
                } else if let videoURL = dict["videoURL"] as? String,
                    let thumbnailImage = dict["thumbnailURL"] as? String,
                    let thumbnailAspect = dict["thumbnailAspect"] as? Double {
