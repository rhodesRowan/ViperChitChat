...
class UserQueryManager {
    ...
    public func fetchUsersForSearching(completion: @escaping (_ userArray: [ChitChatUser]) -> (Void)) {
        var usersArray: [ChitChatUser] = []
                ...
                for document in snapshot!.documents {
                    if document.documentID == Auth.auth().currentUser?.uid {
                        ...
                        if let photoURL = userDict["photoURL"] as? String {
                            let appUser = ChitChatUser(id: document.documentID, name: name, email: email, photoURL: photoURL)
                            usersArray.append(appUser)
                        } else {
                            let appUser = ChitChatUser(id: document.documentID, name: name, email: email)
                            usersArray.append(appUser)
                        }
                    }
                }
    ...
    public func fetchUsersForSearchingByName(searchText: String, completion: @escaping (_ userArray: [ChitChatUser]) -> (Void)) {
        var usersArray: [ChitChatUser] = []
    ...
    ///   - completion: the user
    public func fetchUser(userID: String, completion: @escaping (_ user: ChitChatUser?) -> Void) {
        firestoreRef.collection("users").document(userID).getDocument { (snapshot, error) in
            ...
            let fetchedUser = ChitChatUser(id: userID, name: name, email: email, photoURL: photoURL)
